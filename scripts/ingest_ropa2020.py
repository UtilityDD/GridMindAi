...
def ingest_single_file(i, supabase_url, supabase_key):
    ...
    md_path = rf"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\ROPA2020 ({i}).md"
    if not os.path.exists(md_path):
        return f"File ({i}) not found"

    entry = {
        "ref": f"FIN-ROPA2020-{i}",
        "date": "2020",
        "title": f"ROPA 2020 ({i})",
        "source_url": f"https://github.com/smartlinemanapp/GridMind/blob/main/ROPA2020%20({i}).pdf",
        "keywords": "Finance, ROPA, 2020, WBSEDCL, Salary Revision"
    }
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(ingest_single_file, i, supabase_url, supabase_key): i for i in range(1, 30)}
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Failed ROPA2020 ({idx}): {e}")
