...
def ingest_single_file(i, supabase_url, supabase_key):
    ...
    md_path = rf"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\ROPA2009 ({i}).md"
    if not os.path.exists(md_path):
        return f"File ({i}) not found"

    entry = {
        "ref": f"FIN-ROPA2009-{i}",
        "date": "2009",
        "title": f"ROPA 2009 ({i})",
        "source_url": f"https://github.com/smartlinemanapp/GridMind/blob/main/ROPA2009%20({i}).pdf",
        "keywords": "Finance, ROPA, 2009, WBSEDCL, Salary Revision"
    }
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(ingest_single_file, i, supabase_url, supabase_key): i for i in range(1, 54)}
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Failed ROPA ({idx}): {e}")
