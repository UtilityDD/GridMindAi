...
def main():
    md_path = r"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\Office_Orders_Circulars_Shared.md"
    if not os.path.exists(md_path):
        logger.error(f"File not found: {md_path}")
        return

    entry = {
        "ref": "FIN-OFF-SHARED-MASTER",
        "date": "2024",
        "title": "Office Orders & Circulars Shared - Master Guide",
        "source_url": "https://github.com/smartlinemanapp/GridMind/blob/main/Office_Orders_Circulars_Shared.pdf",
        "keywords": "Finance, Office Order, Circular, Shared, Master Guide, WBSEDCL"
    }

    doc_id = _make_doc_id(entry)
    logger.info(f"Ingesting Master Office Order... (doc_id={doc_id})")
    ...
    logger.info("Completed Master Office Order Ingestion")
