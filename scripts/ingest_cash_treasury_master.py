...
def main():
    md_path = r"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\Cash & Treasury.md"
    if not os.path.exists(md_path):
        logger.error(f"File not found: {md_path}")
        return

    entry = {
        "ref": "FIN-CIRC-CASH-TREASURY-MASTER",
        "date": "2024",
        "title": "Cash & Treasury - Master Guide",
        "source_url": "https://github.com/smartlinemanapp/GridMind/blob/main/Cash%20%26%20Treasury.pdf",
        "keywords": "Finance, Cash, Treasury, Policy, WBSEDCL"
    }

    doc_id = _make_doc_id(entry)
    logger.info(f"Ingesting Master Cash & Treasury... (doc_id={doc_id})")
    ...
    logger.info("Completed Master Cash & Treasury Ingestion")
