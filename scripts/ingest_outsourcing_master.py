...
def main():
    md_path = r"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\Outsourcing_LT_HT.md"
    if not os.path.exists(md_path):
        logger.error(f"File not found: {md_path}")
        return

    entry = {
        "ref": "FIN-OUTSOURCE-LTHT-MASTER",
        "date": "2024",
        "title": "Outsourcing LT & HT - Master Guide",
        "source_url": "https://github.com/smartlinemanapp/GridMind/blob/main/Outsourcing_LT_HT.pdf",
        "keywords": "Finance, Outsourcing, LT, HT, Master Guide, WBSEDCL"
    }

    doc_id = _make_doc_id(entry)
    logger.info(f"Ingesting Master Outsourcing LT & HT... (doc_id={doc_id})")
    ...
    logger.info("Completed Master Outsourcing Ingestion")
