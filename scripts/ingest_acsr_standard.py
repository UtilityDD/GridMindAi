...
import config
# Use only verified functional keys from diagnostic
config.GEMINI_KEY_POOL = [
    config.GEMINI_KEY_POOL[0],
    config.GEMINI_KEY_POOL[1],
    config.GEMINI_KEY_POOL[2],
    config.GEMINI_KEY_POOL[4],
    config.GEMINI_KEY_POOL[7]
]
config.EMBEDDING_BATCH_SIZE = 5 # Smaller batches for less pressure
...
def main():
    ...
    # 4. Embedding
    logger.info(f"Embedding {len(chunks)} chunks using Gemini Pool (8 keys). This may take some time...")
    try:
        # The pipeline/embed.py handles key rotation and rate limiting
