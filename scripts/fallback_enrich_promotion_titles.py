...
def main():
    ...
    # 2. Sequential Enrichment (Chunk based + Gemini 1.5 Fallback)
    results = []
    ...
    for record in records:
        ...
        # b. Generate New Title (Patient Retry with 1.5-flash)
        max_llm_retries = len(config.GEMINI_KEY_POOL)
        new_title = original_title
        
        prompt = f"""
        Document Chunk: {chunk_content[:1500]}
        Current Title: {original_title}
        TASK: Extract the specific subject of this WBSEDCL Promotion Guideline (e.g., "Promotion to SE" or "Salary Fixation").
        ...
        for attempt in range(max_llm_retries):
            client = _get_client()
            try:
                # Force gemini-1.5-flash to bypass 2.0-flash quota
                ...
                if "429" in msg or "RESOURCE_EXHAUSTED" in msg:
                    logger.warning(f"Rate limit hit with 1.5-flash. Sleeping 30s... ({attempt+1})")
                    time.sleep(30)
                    continue
                logger.error(f"LLM fail for {original_title}: {msg[:100]}")
                break
