...
def ingest_single_file(i, supabase_url, supabase_key):
    ...
    md_path = rf"D:\Dipankar\MyCodes\AI Projects\finance_circulars_markdown\Outsourcing_LT_HT ({i}).md"
    if not os.path.exists(md_path):
        return f"File ({i}) not found"

    entry = {
        "ref": f"FIN-OUTSOURCE-LTHT-{i}",
        "date": "2024",
        "title": f"Outsourcing LT & HT ({i})",
        "source_url": f"https://github.com/smartlinemanapp/GridMind/blob/main/Outsourcing_LT_HT%20({i}).pdf",
        "keywords": "Finance, Outsourcing, LT, HT, WBSEDCL"
    }
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(ingest_single_file, i, supabase_url, supabase_key): i for i in range(28, 29)}
